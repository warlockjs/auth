...
const CLIENT_ID = "discord-client-123";
...
async function startLogin() {
  ...
  const url = new URL(await startProviderLogin(response as never, "discord"));
...
describe("startProviderLogin (discord)", () => {
...
describe("completeProviderLogin (discord)", () => {
  ...
  it("never links or creates on an UNVERIFIED provider email — not even to an existing account with that email", async () => {
    ...
    api.user = { ...api.user, verified: false };
    const login = await startLogin();
  ...
  it("an existing link resolves the user by provider id, whatever the email says now", async () => {
    ...
    api.user = { ...api.user, email: "new@example.com", verified: false };
    const login = await startLogin();
